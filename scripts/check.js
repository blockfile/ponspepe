...
(async () => {
  ...
  console.log('token      :', config.tokenAddress || '⚠️ MISSING — set TOKEN_ADDRESS (ponspepe)');
  console.log('reward     :', `${config.rewardSymbol} ${config.rewardToken} (Uniswap V3, fee ${config.rewardPoolFee})`);
  console.log('split      :', `${config.rewardBuyPct}% → buy ${config.rewardSymbol} (airdropped) / ${config.devPct}% dev+gas`);
